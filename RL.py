...
def resample_to_candles(source_folder: str, target_folder: str, frequency: str = '2min'):
    ...
    file_paths = sorted(Path(source_folder).glob("*.csv")) if Path(source_folder).exists() else sorted(Path(PARAMS['SOURCE_FOLDER']).glob("*.csv"))
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage:")
        print("   python script.py train <TICKER>")
        print("   python script.py test <TICKER>")
        print("   python script.py test <TICKER> <day_number>")
        print("   python script.py backtest_ebullient <TICKER>")
        sys.exit(1)
    
    command = sys.argv[1].lower()
    ticker = sys.argv[2].upper()
    
    source_folder = ticker
    target_folder = f"{ticker}_{PARAMS['CANDLE_FREQUENCY']}"
